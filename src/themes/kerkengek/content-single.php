...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="entry-content">
		...
		if ( function_exists( 'wpu_gallery' ) ) {
			wpu_gallery();
		} else if ( function_exists( 'kg_gallery' ) ) {
			kg_gallery( 'div' );
		}
	...
	<footer class="entry-meta">
		...
		if ( ! bushwick_categorized_blog() ) : // This blog only has 1 category so we just need to worry about tags in the meta text.
		{
			if ( '' != $tag_list ) :
		...
		} else : // But this blog has loads of categories so we should probably display them here.
		{
			if ( '' != $tag_list ) :
